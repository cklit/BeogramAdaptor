# BeogramAdaptor
This project uses an ESP32 to connect to a Bang & Olufsen Mozart Platform-based product, and will send simple commands to a Beogram player whenever Line-in is selected. See more info below.


### Compatible with all Bang & Olufsen Mozart Platform-based products that feature a Line-in source * / **
...
_*** If you want to use a Beogram record player without a built-in RIAA pre-amp, you will need a pre-amp that can pass-through the Data Link data-signal. I am currently only aware of this one: https://shop.oneremote.dk/shop/69068-riaa-forstaerker/4199-riaa-ii/ and I have not tested the functionality. Instead of using pin 2 on the DIN plug for data ground, you will need to connect the data ground to the shielding of the DIN-plug instead._
...
![Diagram](/screenshots/breadboard_diagram.png)
...
The ESP32 now will monitor the event stream from the Mozart-based product.

<img src="/screenshots/Connected.png" width="500">
...
You can start the Beogram player from the Mozart Platform-based product, the app, a Beoremote Halo and/or a Beoremote One BT by selecting the **Line-in** source.
...
### Controls directly on the Mozart product: 
Play, Pause, Next and previous works directly on the Mozart product (the < and > buttons are turned off when using Line-in, but they will still work). 
